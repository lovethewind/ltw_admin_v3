...
const hasSelectedTagActions = computed(
  () =>
    selectedTag.value &&
    ['content:tag:create', 'content:tag:update', 'content:tag:delete'].some(
      (code) => canAccess(code),
    ),
);
...
async function handleDelete(row: AdminTag | null = selectedTag.value) {
  if (!row) {
    return;
  }
    ...
    <div class="grid gap-4 lg:grid-cols-[minmax(320px,420px)_1fr]">
      <NCard :bordered="false" title="标签树">
        <template #header-extra>
          <NButton
            v-if="canAccess('content:tag:create')"
            type="primary"
            @click="openCreate()"
          >
            新增根标签
        ...
        <NCard :bordered="false" title="标签详情">
          <template v-if="selectedTag" #header-extra>
            <NSpace v-if="hasSelectedTagActions">
              ...
              <NButton
                v-if="canAccess('content:tag:update')"
                @click="openEdit()"
              >
                编辑
              </NButton>
              <NButton
                v-if="canAccess('content:tag:delete')"
                type="error"
                @click="handleDelete()"
              >
                删除
      ...
      <template #footer>
        <NSpace justify="end">
          ...
          <NButton
            v-if="
              editingId
                ? canAccess('content:tag:update')
                : canAccess('content:tag:create')
            "
            type="primary"
            @click="handleSubmit"
          >
