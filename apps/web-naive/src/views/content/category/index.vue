...
const columns = computed<DataTableColumns<AdminCategory>>(() => [
  ...
  },
  {
    ...
    render: (row) => {
      ...
      if (canAccess('content:category:update')) {
        actions.push(
          h(
            NButton,
      ...
      if (canAccess('content:category:delete')) {
        actions.push(
          h(
            NButton,
  ...
  <Page title="分类管理">
    <NCard :bordered="false">
      <template #header-extra>
        <NButton
          v-if="canAccess('content:category:create')"
          type="primary"
          @click="openCreate"
        >
          新增分类
      ...
      <template #footer>
        <NSpace justify="end">
          ...
          <NButton
            v-if="
              editingId
                ? canAccess('content:category:update')
                : canAccess('content:category:create')
            "
            type="primary"
            @click="handleSubmit"
          >
