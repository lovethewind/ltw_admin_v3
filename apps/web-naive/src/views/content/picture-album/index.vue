...
const columns = computed<DataTableColumns<AdminPictureAlbum>>(() => [
  ...
  { key: 'createTime', title: '创建时间', width: 170 },
  {
    ...
    render: (row) => {
      ...
      if (canAccess('content:pictureAlbum:update')) {
        actions.push(
          h(
            NButton,
      ...
      if (canAccess('content:pictureAlbum:status') && row.status !== 1) {
        actions.push(
          h(
            NButton,
            {
      ...
      if (canAccess('content:pictureAlbum:status') && row.status !== 3) {
        actions.push(
          h(
            NButton,
            {
      ...
      if (canAccess('content:pictureAlbum:delete')) {
        actions.push(
          h(
            NButton,
    ...
    <NCard :bordered="false">
      <div class="admin-filter-toolbar mb-4">
        <NSpace align="center" class="admin-filter-fields" wrap>
          ...
          />
          <NButton
            v-if="canAccess('content:pictureAlbum:query')"
            type="primary"
            @click="handleSearch"
          >
            查询
        ...
        <div class="admin-filter-actions">
          <NButton
            v-if="canAccess('content:pictureAlbum:create')"
            type="primary"
            @click="openCreate"
          >
            新增图册
      ...
      <template #footer>
        <NSpace justify="end">
          ...
          <NButton
            v-if="
              editingId
                ? canAccess('content:pictureAlbum:update')
                : canAccess('content:pictureAlbum:create')
            "
            type="primary"
            @click="handleSubmit"
          >
