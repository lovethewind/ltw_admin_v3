...
const authorOptions = computed(() =>
  authors.value.map((item) => ({
    label: getAuthorSelectLabel(item),
    value: item.id,
  })),
);
...
function getAuthorSelectLabel(author: AdminUser) {
  const name = author.nickname || author.username || '未命名用户';
  return `${name}（${getArticleAuthorUidLabel(author)}）`;
}

function renderAuthorUidTag(author: AdminUser) {
  return h(
    NTag,
    { bordered: false, size: 'small', type: 'info' },
    { default: () => getArticleAuthorUidLabel(author) },
  );
}

function renderAuthorOptionLabel(option: SelectOption) {
  const authorId = (option.value ?? '') as SnowflakeId;
  const author = authorMap.value.get(authorId);
  if (!author) {
  ...
  return h(
    ...
    {
      default: () => [
        h('span', author.nickname || author.username || '未命名用户'),
        renderAuthorUidTag(author),
      ],
    },
  );
}
...
const columns = computed<DataTableColumns<AdminArticle>>(() => [
  ...
  },
  {
    ...
    render: (row) => {
      ...
      if (canAccess('content:article:update')) {
        actions.push(
          h(
            NButton,
      ...
      if (canAccess('content:article:publish') && row.status !== 2) {
        actions.push(
          h(
            NButton,
            {
      ...
      if (canAccess('content:article:delete')) {
        actions.push(
          h(
            NButton,
    ...
    <NCard :bordered="false">
      <div class="admin-filter-toolbar mb-4">
        ...
        <div class="admin-filter-actions">
          <NButton
            v-if="canAccess('content:article:create')"
            type="primary"
            @click="openCreate"
          >
            新增文章
        ...
        <div class="grid grid-cols-1 gap-x-4 md:grid-cols-2">
          ...
          <NFormItem label="作者" path="userId">
            <NSelect
              ...
              filterable
              placeholder="请选择作者"
              :render-label="renderAuthorOptionLabel"
            />
          </NFormItem>
      ...
      <template #footer>
        <NSpace justify="end">
          ...
          <NButton
            v-if="
              currentArticleId
                ? canAccess('content:article:update')
                : canAccess('content:article:create')
            "
            type="primary"
            @click="handleSubmit"
          >
