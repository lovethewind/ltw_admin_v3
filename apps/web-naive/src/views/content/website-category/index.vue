...
const columns = computed<DataTableColumns<AdminWebsiteCategory>>(() => [
  ...
  { key: 'updateTime', title: '更新时间', width: 170 },
  {
    ...
    render: (row) => {
      ...
      if (canAccess('content:websiteCategory:update')) {
        actions.push(
          h(
            NButton,
      ...
      if (canAccess('content:websiteCategory:delete')) {
        actions.push(
          h(
            NButton,
  ...
  <Page title="导航分类">
    <NCard :bordered="false">
      <NSpace justify="space-between" class="mb-4">
        <span></span>
        <NButton
          v-if="canAccess('content:websiteCategory:create')"
          type="primary"
          @click="openCreate"
        >
          新增分类
      ...
      <template #footer>
        <NSpace justify="end">
          ...
          <NButton
            v-if="
              editingId
                ? canAccess('content:websiteCategory:update')
                : canAccess('content:websiteCategory:create')
            "
            type="primary"
            @click="handleSubmit"
          >
