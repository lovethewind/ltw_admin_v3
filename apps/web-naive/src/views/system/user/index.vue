...
const columns = computed<DataTableColumns<AdminUser>>(() => [
  ...
  },
  {
    ...
    render: (row) => {
      ...
      if (canAccess('system:user:update')) {
        actions.push(
          h(
            NButton,
      ...
      if (canAccess('system:user:delete')) {
        actions.push(
          h(
            NButton,
    ...
    <NCard :bordered="false">
      <div class="admin-filter-toolbar mb-4">
        ...
        <div class="admin-filter-actions">
          <NButton
            v-if="canAccess('system:user:create')"
            type="primary"
            @click="openCreate"
          >
            新增用户
    ...
    >
      ...
      <template #footer>
        <NSpace justify="end">
          ...
          <NButton
            v-if="canAccess('system:user:role')"
            type="primary"
