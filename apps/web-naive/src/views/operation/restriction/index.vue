...
const columns = computed<DataTableColumns<AdminUserRestriction>>(() => [
  ...
  { key: 'createTime', title: '创建时间', width: 170 },
  {
    ...
    render: (row) => {
      ...
      if (canAccess('operation:restriction:update'))
        actions.push(
          h(
            NButton,
      ...
      if (canAccess('operation:restriction:cancel') && !row.isCancel)
        actions.push(
          h(
            NButton,
            {
      ...
      if (canAccess('operation:restriction:delete'))
        actions.push(
          h(
            NButton,
      ...
      <div class="admin-filter-toolbar mb-4">
        <NSpace align="center" class="admin-filter-fields" wrap>
          ...
          /><NSelect
            ...
            class="w-[120px]"
          /><NButton
            v-if="canAccess('operation:restriction:query')"
            type="primary"
            @click="handleSearch"
          >
            查询
        ...
        <div class="admin-filter-actions">
          <NButton
            v-if="canAccess('operation:restriction:create')"
            type="primary"
            @click="openCreate"
          >
            新增限制
