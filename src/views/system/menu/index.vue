...
const hasSelectedMenuActions = computed(
  () =>
    ...
    getPermittedActions(accessStore.accessCodes, [
      { code: 'menu:create', key: 'create' },
      { code: 'menu:update', key: 'update' },
      { code: 'menu:delete', key: 'delete' },
    ]).length > 0,
);
 ...
 * :return: 异步删除处理结果
 */
function handleDelete(row: AdminMenu | null = selectedMenu.value): void {
  if (!row) {
    return;
  }

  const deleteKeys = collectMenuDeleteKeys(row);
  const childCount = deleteKeys.length - 1;
  dialog.warning({
    title: '删除菜单',
    content:
      childCount > 0
        ? `确认删除“${row.name}”？将同时删除 ${childCount} 个子菜单。`
        : `确认删除“${row.name}”？`,
    positiveText: '确认删除',
    negativeText: '取消',
      ...
      <template #footer>
        <NSpace justify="end">
          ...
          <NButton
            v-if="
              editingId
                ? canAccess('menu:update')
                : canAccess('menu:create')
            "
            type="primary"
            @click="handleSubmit"
          >
